...
class TradingStatistics:
    ...
    def print_stats(self):
        ...
        logging.info(
            f"Taker交易: {self.taker_trades} ({100-self.get_maker_ratio():.1f}%)"
        )

        # 费用优化建议
        target_maker_ratio = (
            STRATEGY_CONFIG["FEE_OPTIMIZATION"]["TARGET_MAKER_RATIO"] * 100
        )
        current_maker_ratio = self.get_maker_ratio()

        if current_maker_ratio < target_maker_ratio:
            logging.warning(
                f"[费用优化] Maker比例 ({current_maker_ratio:.1f}%) "
                f"低于目标 ({target_maker_ratio:.1f}%)"
            )
            logging.warning("[费用优化] 建议: 增加限价单等待时间或调整价格偏移")
        else:
            logging.info(
                f"[费用优化] Maker比例达标 "
                f"({current_maker_ratio:.1f}% >= {target_maker_ratio:.1f}%)"
            )
...
class RiskManager:
    ...
    def __init__(self, stats):
        ...
        self.max_drawdown_percent = TRADING_CONFIG.get(
            "MAX_POSITION_DRAWDOWN", 0.06
        )  # 从配置读取，默认6%
        self.drawdown_warning_threshold = TRADING_CONFIG.get(
            "DRAWDOWN_WARNING_THRESHOLD", 0.048
        )  # 从配置读取
        self.force_close_enabled = TRADING_CONFIG.get(
    ...
    def update_position_value(self, symbol, current_price, position_size):
        ...
        logging.debug(
            f"[风险管理] {symbol} 当前回撤: {drawdown_percent:.2f}% "
            f"(峰值: {peak_value:.2f}, 当前: {current_value:.2f})"
        )

        # 检查回撤限制
        if drawdown > self.max_drawdown_percent:
            logging.error(
                f"[风险管理] {symbol} 仓位回撤超限: {drawdown_percent:.2f}% > "
                f"{self.max_drawdown_percent*100:.1f}%"
            )
            logging.error(
                f"[风险管理] 峰值价值: {peak_value:.2f} USDT, "
                f"当前价值: {current_value:.2f} USDT"
            )
        ...
        if drawdown > self.drawdown_warning_threshold:
            logging.warning(
                f"[风险管理] {symbol} 仓位回撤接近限制: {drawdown_percent:.2f}% "
                f"(警告阈值: {self.drawdown_warning_threshold*100:.1f}%, "
                f"限制: {self.max_drawdown_percent*100:.1f}%)"
            )
...
class EnhancedAsterDexAPI:
            ...
            try:
                ...
                if response.status_code == 400:
                    ...
                    try:
                        ...
                        logging.error(
                            f"[API] 400错误详情 - 代码: {error_code}, 消息: {error_msg}"
                        )
    ...
    def place_market_order(self, symbol, side, quantity, reduce_only=False):
        ...
        return self.place_order(
            symbol, side, "MARKET", quantity, reduce_only=reduce_only
        )

    def place_limit_order(
        self, symbol, side, quantity, price, time_in_force="IOC", reduce_only=False
    ):
        """下限价单"""
        return self.place_order(
            symbol, side, "LIMIT", quantity, price, time_in_force, reduce_only
        )
...
class EnhancedSpreadMonitor:
    ...
    def start_monitoring(self):
        ...
        ws_options = {
            ...
            if not WEBSOCKET_CONFIG.get("VERIFY_SSL", True)
            else {},
        }
...
class EnhancedVolumeTrader:
    ...
    def __init__(self):
        ...
        # 新增：趋势分析组件
        self.trend_analyzer = TrendAnalyzer(window_size=30, short_ma=5, long_ma=20)
        ...
        # 趋势交易配置
        self.trend_trading_enabled = STRATEGY_CONFIG.get("TREND_TRADING", {}).get(
            "ENABLED", True
        )
        self.trend_confidence_threshold = STRATEGY_CONFIG.get("TREND_TRADING", {}).get(
            "MIN_CONFIDENCE", 0.2
        )
        self.directional_trade_ratio = STRATEGY_CONFIG.get("TREND_TRADING", {}).get(
            ...
            if position_info:
                for position in position_info:
                    if position["symbol"] == symbol:
                        ...
                        if abs(position_amt) > 0.001:
                            # 检查仓位回撤
                            if not self.risk_manager.update_position_value(
                                symbol, current_price, position_amt
                            ):
                                logging.error(f"[风险管理] {symbol} 仓位回撤超限，执行强制平仓")
                                if self.risk_manager.force_close_position(
                                    symbol, self.api
                                ):
    ...
    def update_trend_data(self, symbol, spread_data):
        ...
        if new_direction != self.current_trend_direction:
            logging.info(
                f"[趋势变化] {symbol} 趋势从 {self.current_trend_direction} 变为 {new_direction}"
            )
        ...
        try:
            ...
            if self.trend_trading_enabled and trade_direction in ["LONG", "SHORT"]:
                ...
                (
                    ...
                    is_maker,
                    estimated_fee,
                ) = self.execute_traditional_volume_trade(symbol, bid, ask, spread_data)

            # 更新统计
            self.stats.add_trade(
                success, self.trade_quantity, 0, is_maker, estimated_fee
            )
    ...
    def execute_directional_trade(self, symbol, direction, bid, ask, spread_data):
        ...
        try:
            if direction == "LONG":
                ...
                return self.execute_traditional_volume_trade(
        ...
        try:
            ...
            if not sell_order or "orderId" not in sell_order:
                ...
                self.positions[symbol] = (
                    self.positions.get(symbol, 0) + self.trade_quantity
                )
                return False, False, 0
            ...
            # 等待卖出订单成交 (可以等待更长时间)
            sell_filled = self.wait_for_order_fill(
                symbol, sell_order_id, max_wait_time * 2
            )
            ...
            if not sell_filled:
                ...
                try:
                    ...
                    market_sell = self.api.place_market_order(
                        symbol, "SELL", self.trade_quantity, reduce_only=True
                    )
                    if market_sell:
            ...
            estimated_fee = (
                self.trade_quantity * (buy_price + sell_price) * 0.0002 * 0.8
            )  # 80% Maker概率
    ...
    def execute_short_strategy(self, symbol, bid, ask, spread_data):
        ...
        try:
            ...
            # 等待买入订单成交
            buy_filled = self.wait_for_order_fill(
                symbol, buy_order_id, max_wait_time * 2
            )
            ...
            if not buy_filled:
                ...
                try:
                    self.api.cancel_order(symbol, buy_order_id)
                    market_buy = self.api.place_market_order(
                        symbol, "BUY", self.trade_quantity, reduce_only=True
                    )
                    if market_buy:
                        logging.info(f"[做空] 使用市价单平仓: {symbol}")
                        estimated_fee = (
                            self.trade_quantity * sell_price * 0.0003
                        )  # 混合费率
                        return True, False, estimated_fee
                except:
                    pass
    ...
    def use_optimized_limit_strategy(self, symbol, bid, ask, spread_data):
        ...
        try:
            ...
            sell_offset = STRATEGY_CONFIG["LIMIT_ORDER"]["SELL_PRICE_OFFSET"]
            time_in_force = STRATEGY_CONFIG["LIMIT_ORDER"]["TIME_IN_FORCE"]
            max_wait_time = STRATEGY_CONFIG["LIMIT_ORDER"].get(
                "MAX_WAIT_TIME", 5
            )  # 最大等待时间
    ...
    def get_account_status(self):
        ...
        try:
            ...
            if balance_info and isinstance(balance_info, list):
                ...
                logging.info(
                    f"[API] 调用次数: {self.api.api_calls}, 错误次数: {self.api.api_errors}"
                )
                return balance_info
            else:
                # 如果余额API失败，尝试账户信息API
            ...
            while self.trading_enabled and not self.risk_manager.should_stop_trading():
                ...
                if not self.spread_monitor.is_healthy():
                    ...
                    monitor_thread = threading.Thread(
                        target=self.spread_monitor.start_monitoring
                    )
                    monitor_thread.daemon = True
                    monitor_thread.start()
